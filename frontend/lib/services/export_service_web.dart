...
Future<void> exportToPDF() async {
  ...
  ..writeln('- الإيرادات الكلية: ${_asDouble(analytics['totalRevenue']).toStringAsFixed(2)} ج.م')
  ..writeln('- التكاليف الكلية: ${_asDouble(analytics['totalCost']).toStringAsFixed(2)} ج.م')
  ..writeln('- صافي الربح: ${_asDouble(analytics['netProfit']).toStringAsFixed(2)} ج.م')
    ..writeln('- هامش الربح: ${_asDouble(analytics['profitMargin']).toStringAsFixed(1)}%')
    ..writeln()
    ..writeln('🔮 التنبؤات:')
  ..writeln('- الإيرادات المتوقعة: ${_asDouble(predictions['predictedRevenue']).toStringAsFixed(2)} ج.م')
  ...
  // إنشاء ملف PDF وتنزيله باستخدام dart:html (ويب فقط)
...
Future<void> exportToExcel() async {
  ...
  add('الإيرادات الكلية', '${_asDouble(analytics['totalRevenue']).toStringAsFixed(2)} ج.م');
  add('التكاليف الكلية', '${_asDouble(analytics['totalCost']).toStringAsFixed(2)} ج.م');
  add('صافي الربح', '${_asDouble(analytics['netProfit']).toStringAsFixed(2)} ج.م');
  ...
  sheet.getRangeByName('B$row').setText('السعر (ج.م)');
  ...
  // حفظ ملف Excel وتنزيله باستخدام dart:html (ويب فقط)
